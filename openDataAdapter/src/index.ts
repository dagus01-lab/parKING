...
const opendataApiUrl ="https://opendata.comune.bologna.it/api/explore/v2.1/catalog/datasets/disponibilita-parcheggi-vigente/records?where=parcheggio%3D%22Riva%20Reno%22&limit=20"
      ...
      fetch(parKingApiUrl + "?id=" + parkingLotId, {
        ...
        body: JSON.stringify(
          new ParkingLot(
            parkingLotId,
            p.parcheggio,
            { latitude: p.coordinate.lat, longitude: p.coordinate.lon },
            p.posti_totali,
            p.posti_liberi,
            Date.parse(p.data)
          )
