...
import { GetParkingLotsRequest, PostParkingLotsRequest } from "../api/api.parkingLots.types";
...
const parkingLots = new Map<number, ParkingLot>([
    [1, new ParkingLot(1, "Riva Reno", { latitude: 44.504422, longitude: 11.346514 }, 470, 470, 1732451330349)],
]);
...
export function getParkingLots(req: GetParkingLotsRequest, res: Response) {
    ...
    from(parkingLots.values()).pipe(
        filter(pl => { return calcCrow(boundary.center, pl.coordinate) < boundary.radius }),
        take(min(maxResults, DEFAULT_MAX_RESULTS))
    ).forEach(pl =>
        result.push(pl)

    )
    console.log(`get request for at most ${maxResults} parking lots in boundary: ${JSON.stringify(boundary)}\nresults:${JSON.stringify(result)}`);
    res.json(result);
}

export function putParkingLots(req: PostParkingLotsRequest, res: Response) {
    const id = Number.parseInt(req.query["id"]);
    const oldValue = parkingLots.get(id);
    parkingLots.set(id, req.body);
    console.log(`put request for parking lot with id:${id}\nold value:${JSON.stringify(oldValue)}\nnew value:${JSON.stringify(req.body)}`);
    res.json(req.body);
}